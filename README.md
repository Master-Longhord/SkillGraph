...
- **Visual Graph Explorer**: 2D HTML5 Canvas force-directed graph renderer with zoom/pan, center isolation, node filtering, and relationship labels.
- **Shortest Path Finder**: Execute openCypher `shortestPath()` queries between any two graph entities.
- **Robust State Management**: Full error handling, database connectivity status indicators, loading skeletons, and graceful fallback states.
